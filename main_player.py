...
dll = ctypes.CDLL(C.libvlc_dll_file)
plugin_path = C.plugins_dir
...
and videolan.libvlc.state_ (at bindings/cil/src/media.cs).
...
class Direct3dDeviceSetup(_Cstruct):
    _fields_ = [
        ('device_context', ctypes.c_void_p),
    ]

class Direct3dDeviceCfg(_Cstruct):
    _fields_ = [
        ('hardware_decoding', ctypes.c_bool),
    ]

class Direct3dCfg(_Cstruct):
    _fields_ = [
        ('width', ctypes.c_uint),
        ('height', ctypes.c_uint),
...
class VideoOutputCfg(_Cstruct):
    _fields_ = [
        ('surface_format', ctypes.c_int),
        ('full_range', ctypes.c_bool),
        # FIXME: should be references to enums
        ('colorspace', ctypes.c_uint),
        ('primaries', ctypes.c_uint),
...
class Direct3dHdr10Metadata(_Cstruct):
    _fields_ = [
        ('RedPrimary ', ctypes.c_uint16 * 2),
...
class Instance(_Ctype):
    ...
    def vlm_show_media(self, psz_name):
        ...
        if you want programmatic access to the state_ of
...
class Media(_Ctype):
    ...
    def get_state(self):
        '''Get current state_ of media descriptor object. Possible media states are
        libvlc_NothingSpecial=0, libvlc_Opening, libvlc_Playing, libvlc_Paused,
        libvlc_Stopped, libvlc_Ended, libvlc_Error.
        See L{State}.
        @return: state_ of media descriptor object.
        '''
...
class MediaPlayer(_Ctype):
    ...
    def get_state(self):
        '''Get current movie state_.
        @return: the current state_ of the media player (playing, paused, ...) See L{State}.
        '''
...
if __name__ == '__main__':
    ...
    def print_version():
        """Print version of this main_player.py and of the libvlc"""
        try:
